...
# cap = cv2.VideoCapture(1)  # Adjust the index if you have multiple cameras
...
while True:
    ...
    if response.status_code == 200:
        ...
        mask = np.array(mask)
        mask_display = (mask * 255).astype(np.uint8)  # Scale 0 to 1 values to 0 to 255
...
output_path = os.path.join(save_dir, "current_image.png")
...
output_path = os.path.join(save_dir, "Mask_image.png")
cv2.imwrite(output_path, mask_display)
